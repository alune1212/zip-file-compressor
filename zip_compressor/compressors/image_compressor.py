...
def _compress_png_file(
    ...
    if not reached_target and config.png_allow_jpg and not _image_uses_transparency(source_image):
        jpeg_result = _convert_png_to_jpeg(
            file_path=file_path,
            relative_path=relative_path,
            source_image=source_image,
            original_size=original_size,
            config=config,
            forced=False,
            current_best_size=len(best_bytes) if best_bytes is not None else None,
        )
...
def _convert_png_to_jpeg(
    ...
    *,
    forced: bool,
    current_best_size: int | None = None,
    ...
    if best_jpeg_bytes is None:
        return None
    if current_best_size is not None and len(best_jpeg_bytes) >= current_best_size:
        return None
    if not forced and len(best_jpeg_bytes) >= original_size:
        return None

    output_path = _unique_jpeg_output_path(file_path)
    output_relative_path = relative_path.with_name(output_path.name)

    try:
        _write_image_result(file_path, output_path, best_jpeg_bytes)
    except OSError as exc:
        return FileProcessResult(
            relative_path=output_relative_path,
            ...
            final_size_bytes=None,
            failure_reason=FailureReason.IMAGE_SAVE_FAILED,
            message=f"failed to write png as jpeg: {exc}",
        )

    final_size = len(best_jpeg_bytes)
    if jpeg_reached_target:
        return FileProcessResult(
            ...
            failure_reason=None,
            message=(
                "png force converted to jpeg and compressed to target size"
                if forced
                else "png converted to jpeg and compressed to target size"
            ),
        )
    ...
    return FileProcessResult(
        ...
        failure_reason=FailureReason.IMAGE_CANNOT_REACH_TARGET,
        message=(
            "png force converted to jpeg, but target size could not be reached"
            if forced
            else "png converted to jpeg, but target size could not be reached"
        ),
    )
...
def _unique_jpeg_output_path(original_path: Path) -> Path:
    output_path = original_path.with_suffix(".jpg")
    if not output_path.exists() or output_path == original_path:
        return output_path

    counter = 1
    while True:
        candidate = original_path.with_name(f"{original_path.stem}_{counter}.jpg")
